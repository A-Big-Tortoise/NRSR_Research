...
def performDBA_(series, n_iterations=10):
    ...
    for i in range(0,n_iterations):
        center, updated_center_ls = DBA_update(center, series, cost_mat, path_mat, delta_mat)

    tolerance = np.array([np.std(updated_center) for updated_center in updated_center_ls])
...
def DBA_update(center, series, cost_mat, path_mat, delta_mat):
    ...
    updated_center_ls = [[] for _ in range(len(center))]
    ...
    for s in series:
        ...
        while(path_mat[i, j] != -1):
            ...
            updated_center_ls[i].append(s[j])
            n_elements[i] += 1
    ...
    return np.divide(updated_center, n_elements), updated_center_ls
...
def main():
    ...
    average_series = performDBA_(series)
